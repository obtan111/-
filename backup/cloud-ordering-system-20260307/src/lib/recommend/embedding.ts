...
// 基于购物车内容的推荐
export async function getCartBasedRecommendations(dishIds: number[], limit: number = 6) {
  try {
    const client = getSupabaseClient();
...
// 获取热门菜品
export async function getPopularDishesSimple(limit: number = 10) {
  try {
    const client = getSupabaseClient();
