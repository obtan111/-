...
async function getOrderBasedRecommendations(userId: number, limit: number = 6) {
  try {
    ...
    if (vectorRecommendations.length < limit) {
      for (const categoryId of preferredCategories) {
        const { data: categoryDishes } = await client
          .from('dishes')
          .select('*')
          .eq('category_id', categoryId)
          .eq('is_active', true)
          .not('id', 'in', `(${purchasedDishIds.join(',')})`)
          .not('id', 'in', `(${vectorRecommendations.map(r => r.id).join(',')})`)
          .order('sales', { ascending: false })
          .limit(Math.ceil(limit / preferredCategories.length) + 2);
    ...
    if (allRecommendations.length < limit) {
      const existingIds = [...purchasedDishIds, ...allRecommendations.map(r => r.id)];
      const { data: popularDishes } = await client
        .from('dishes')
        .select('*')
        .eq('is_active', true)
  ...
  try {
    ...
    switch (type) {
      ...
      case 'order_based':
        ...
        recommendations = await getOrderBasedRecommendations(authResultOrder.id, limit);
        if (!recommendations) {
          // 没有订单历史，返回热门菜品
      ...
      case 'cart_based':
        ...
        if (!cartData || !cartData.items || cartData.items.length === 0) {
          ...
          // 使用基于购物车的推荐
          recommendations = await getCartBasedRecommendations(cartDishIds, limit);
        }
        break;
