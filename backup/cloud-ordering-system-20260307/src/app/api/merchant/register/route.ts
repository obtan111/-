...
import { registerMerchant } from '@/lib/auth/auth';

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { username, email, password, shop_name, phone, address, description } = body;

    // 验证必填字段
    if (!username || !email || !password || !shop_name || !phone) {
      return NextResponse.json(
        { success: false, error: '用户名、邮箱、密码、店铺名称和电话为必填项' },
        { status: 400 }
      );
    }
    ...
    if (shop_name.length < 2 || shop_name.length > 100) {
      return NextResponse.json(
        { success: false, error: '店铺名称长度应为2-100个字符' },
        { status: 400 }
      );
    }
